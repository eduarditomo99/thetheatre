...
public class AuthController {
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequest request) {
        try {
            ...
            return ResponseEntity.status(403).body("Credenciales incorrectas o usuario no encontrado");
        }
    }
    ...
    public ResponseEntity<?> register(@RequestBody RegisterRequest request) {
        ...
        if (request.getPassword() == null || !request.getPassword().matches(regex)) {
            return ResponseEntity.badRequest().body("La contraseña es débil. Requiere: Mayúscula, minúscula, número, símbolo y 8 caracteres.");
        }

        Usuario user = new Usuario();
        user.setUsername(request.getUsername());
        // Evitamos nulos en nombre y apellidos
        ...
        try {
            ...
            // Esto captura si el email O el username ya existen
            return ResponseEntity.badRequest().body("Error: El email o el nombre de usuario ya están registrados.");
        } catch (Exception e) {
            // Cualquier otro error interno
            e.printStackTrace(); // Ver error en consola Backend
            return ResponseEntity.internalServerError().body("Error interno al registrar usuario: " + e.getMessage());
        }
...
// --- CLASES DTO (Copia esto también al final del archivo) ---
